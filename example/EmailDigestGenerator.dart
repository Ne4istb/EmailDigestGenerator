...
Client client = new Client(consumerKey, accessToken);
...
main() {
	...
	client.getData(options: options)
...
Map<String, List<Link>> group(items) {
...
Link toLink(PocketData item) {
	...
	return new Link(int.parse(item.itemId), title, item.excerpt, url, tags, imageUrl: imageUrl);
}
...
Map<String, List<Link>> sendEmail(Map<String, List<Link>> links) {
...
cleanUp(Map<String, List<Link>> links) {
	...
	links.values.forEach((group) {
		group.forEach((link) {
			actions.add(new RemoveTagsAction(link.id, ['buggy']));
